...
class OpenAIService {
    constructor() {
        if (!process.env.OPENAI_API_KEY) {
            ...
            this.openai = null; // Will use mock responses
        }
        else {
...
What's your biggest challenge with this? Drop it in the comments below and I'll personally help you out. Don't forget to follow for more life-changing tips!`;
      ...
      Focus on the spoken content only.`;
            ...
            if (!script) {
                throw new errorHandler_1.AppError('Failed to generate script', 500);
            }
            return script.trim();
        }
        catch (error) {
            ...
            throw new errorHandler_1.AppError('Failed to generate script with OpenAI', 500);
        }
    }
    /**
     * Generate captions with timing from script
     */
    async generateCaptions(script, audioDuration) {
        try {
      ...
      - Make sure the last caption ends at exactly ${audioDuration} seconds`;
            ...
            if (!response) {
                throw new errorHandler_1.AppError('Failed to generate captions', 500);
            }
            // Parse JSON response

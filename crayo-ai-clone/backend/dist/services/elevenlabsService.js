...
class ElevenLabsService {
    ...
    async getVoices() {
        try {
            ...
            throw new errorHandler_1.AppError('Failed to fetch voices from ElevenLabs', 500);
        }
    }
    /**
    ...
    async generateVoiceover(request) {
        try {
            ...
            throw new errorHandler_1.AppError('Failed to generate voiceover', 500);
        }
    }
    /**
    ...
    async saveAudioFile(audioBuffer, filename) {
        try {
            const uploadsDir = path_1.default.join(process.cwd(), 'uploads', 'audio');
            // Ensure uploads directory exists
            if (!fs_1.default.existsSync(uploadsDir)) {
                fs_1.default.mkdirSync(uploadsDir, { recursive: true });
            }
            const filePath = path_1.default.join(uploadsDir, `${filename}.mp3`);
            fs_1.default.writeFileSync(filePath, audioBuffer);
            return filePath;
        }
        catch (error) {
            console.error('File save error:', error);
            throw new errorHandler_1.AppError('Failed to save audio file', 500);
        }
    }
    /**
     ...
     */
    estimateAudioDuration(script, speed = 1.0) {

...
router.use('/uploads', express_2.default.static(path_1.default.join(process.cwd(), 'uploads')));
/**
 * GET /api/assets/background-videos
 * Get available background videos
 */
router.get('/background-videos', (req, res) => {
 ...
 */
router.get('/background-videos/search', (req, res) => {
    ...
    const matchingVideos = allVideos.filter(video => searchTerms.some(term => video.title.toLowerCase().includes(term) ||
